...
public class AutoCollectManager {
    ...
    public void toggle() {
        ...
        if (client.player != null) {
            client.player.sendMessage(Text.translatable(isActive.get() ? "msg.elytraautocollect.toggle.on" :
                    "msg.elytraautocollect.toggle.off"), true);
            if (isActive.get()) {
                client.player.sendMessage(Text.translatable("msg.elytraautocollect.session.start"), false);
                validateConfiguration(client);
            } else {
                client.player.sendMessage(Text.translatable("msg.elytraautocollect.session.end", shipsFound.get()), false);
            }
        }
        if (!isActive.get()) {
            resetState();
    ...
    private void validateConfiguration(MinecraftClient client) {
        ...
        client.player.sendMessage(Text.translatable("msg.elytraautocollect.config.header"), false);
        ...
        client.player.sendMessage(Text.translatable("msg.elytraautocollect.config.render", renderDistance, scanRadius), false);
    }
    ...
    public void tick(MinecraftClient client) {
        ...
        if (elytraFound && !landingMode) {
            initiateSafeLanding(client, player, Text.translatable("msg.elytraautocollect.landing.reason.found"));
            return;
        }
    ...
    private boolean performSafetyChecks(MinecraftClient client, ClientPlayerEntity player) {
        if (!flightManager.tryEquipElytra(player)) {
            client.player.sendMessage(Text.translatable("msg.elytraautocollect.safety.noelytra"), false);
            return false;
        }
        if (player.getHealth() < 6.0f) {
            client.player.sendMessage(Text.translatable("msg.elytraautocollect.safety.health"), false);
            return false;
        }
        if (isElytraDurabilityLow(player)) {
            if (!lowDurabilityDetected) {
                if (tryReplaceElytra(client, player)) {
                    client.player.sendMessage(Text.translatable("msg.elytraautocollect.safety.elytra.swap"), false);
                    lowDurabilityDetected = false;
                } else {
                    int percent = getElytraDurabilityPercent(player);
                    client.player.sendMessage(Text.translatable("msg.elytraautocollect.safety.elytra.low", percent), false);
                    lowDurabilityDetected = true;
                    initiateSafeLanding(client, player, Text.translatable("msg.elytraautocollect.landing.reason.dura"));
                    return true;
                }
            }
        }
        if (!hasEnoughFireworks(player)) {
            if (!lowFireworksDetected) {
                ...
                client.player.sendMessage(Text.translatable("msg.elytraautocollect.safety.fireworks.low", count), false);
                lowFireworksDetected = true;
                initiateSafeLanding(client, player, Text.translatable("msg.elytraautocollect.landing.reason.fireworks"));
                return true;
            }
        }
        return true;
    ...
    private void initiateSafeLanding(MinecraftClient client, ClientPlayerEntity player, Text reason) {
        ...
        client.player.sendMessage(Text.translatable("msg.elytraautocollect.landing.head").append(reason), false);
    }
    ...
    private void calculateSafeLandingPosition(MinecraftClient client, ClientPlayerEntity player) {
        ...
        if (best != null) {
            landingTarget = best;
            client.player.sendMessage(Text.translatable("msg.elytraautocollect.landing.found"), false);
        } else {
            landingTarget = findEmergencyLandingSpot(world, playerPos);
            client.player.sendMessage(Text.translatable("msg.elytraautocollect.landing.emergency"), false);
        }
    }
    ...
    private void completeSafeLanding(MinecraftClient client) {
        flightManager.stopMovement(client);
        if (client.player != null) {
            if (lowDurabilityDetected) client.player.sendMessage(Text.translatable("msg.elytraautocollect.landing.done.dura"), false);
            else if (lowFireworksDetected) client.player.sendMessage(Text.translatable("msg.elytraautocollect.landing.done.fireworks"), false);
            else client.player.sendMessage(Text.translatable("msg.elytraautocollect.landing.done.success"), false);
        }
    ...
    private void handleTakeoff(MinecraftClient client) {
        if (flightManager.takeOff(client)) {
            flightState = FlightState.CRUISING;
            if (client.player != null) client.player.sendMessage(Text.translatable("msg.elytraautocollect.takeoff.success"), false);
        }
    }
    ...
    private void handleApproachPurpur(MinecraftClient client) {
        ...
        if (p.getPos().distanceTo(purpurTargetPosition) < 50.0) {
            ...
            if (client.player != null) client.player.sendMessage(Text.translatable("msg.elytraautocollect.purpur.reached"), false);
        } else {
    ...
    public void setPurpurTargetPosition(Vec3d pos) {
        ...
        if (MinecraftClient.getInstance().player != null)
            MinecraftClient.getInstance().player.sendMessage(Text.translatable("msg.elytraautocollect.purpur.found"), false);
    }
